...
MATCH_FORCE=MATCH_FORCE_ENTERPRISE=\"1\"
...
case $key in
    -h|--help)
    echo "bash enterprise_bundle_ID.sh -b com.company.appname.jenkins -a AppName"
    exit 0
    ;;
    -b|--bundleid)
    BUNDLEID=$2
...
fastlane produce --username $USER_NAME --app_identifier $BUNDLEID --app_name $APP_NAME --sku $SKU --skip_itc
fastlane produce enable_services --push-notification --username $USER_NAME --app_identifier $BUNDLEID
...
fastlane match $MATCH_TYPE --git_url $GITURL --app_identifier $BUNDLEID --username $USER_NAME $MATCH_FORCE
# MATCH_FORCE_ENTERPRISE="1" fastlane match enterprise --git_url ssh://git@github.com:Jios/fastlane_script.git --app_identifier com.company.bosma.jenkins --username appleAccount@email.com
